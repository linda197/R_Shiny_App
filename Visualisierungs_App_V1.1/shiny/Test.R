...
ui <- fluidPage(titlePanel("Datum-Auswahl"),
                ...
                sidebarLayout(
                  ...
                  mainPanel(tableOutput("ultraschall_table"))
                ))
...
server <- function(input, output) {
  ...
  Energie_folder <- "C:/Users/Lenovo/Documents/Uni/6.Semester/Projekt/Ordnerstruktur/Ultraschalldaten"
  ...
  get_files <- function(start_date, end_date, data_folder) {
    ...
    for (year_folder in selected_year_folders) {
      ...
      for (month_folder in selected_month_folders) {
        file_paths <-
          list.files(month_folder,pattern = "\\.csv$", full.names = TRUE) 
